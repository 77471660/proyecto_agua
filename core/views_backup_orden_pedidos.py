...
from django.db.models import Sum, Count, Q, Max, Case, When, Value, IntegerField
...
def dashboard(request):
    ...
    if clientes_activos >= 5:
        recomendaciones.append(
            'La cartera de clientes frecuentes estÃ¡ creciendo.'
        )
...
def editar_cliente(request, cliente_id):
    ...
    if request.method == 'POST':
        ...
        if not nombre or not telefono or not direccion:
            messages.error(
                request,
                'Nombre, telÃ©fono y direcciÃ³n son obligatorios.'
            )
            return redirect(
                'editar_cliente',
        ...
        if len(nombre) > 100 or len(telefono) > 20 or len(direccion) > 255:
            messages.error(
                request,
                'Revisa la longitud de nombre, telÃ©fono o direcciÃ³n.'
            )
            return redirect(
                'editar_cliente',
                cliente_id=cliente.id
        ...
        if len(telefono) < 6:
            messages.error(request, 'El telÃ©fono debe tener al menos 6 caracteres.')
            return redirect(
                'editar_cliente',
                cliente_id=cliente.id
            )
        ...
        if telefono_duplicado:
            messages.error(request, 'Ya existe otro cliente con ese telÃ©fono.')
            return redirect(
                'editar_cliente',
                cliente_id=cliente.id
            )
...
def registrar_cliente(request):
    ...
    if request.method == 'POST':
        ...
        if not nombre or not telefono or not direccion:
            messages.error(request, 'Nombre, telÃ©fono y direcciÃ³n son obligatorios.')
            return redirect('registrar_cliente')

        if len(nombre) > 100 or len(telefono) > 20 or len(direccion) > 255:
            messages.error(request, 'Revisa la longitud de nombre, telÃ©fono o direcciÃ³n.')
            return redirect('registrar_cliente')
        ...
        if Cliente.objects.filter(telefono=telefono).exists():
            messages.error(request, 'Ya existe un cliente con ese telÃ©fono.')
            return redirect('registrar_cliente')
...
def registrar_pedido(request):
    ...
    if request.method == 'POST':
        ...
        if not cliente_id.isdigit():
            messages.error(request, 'Cliente seleccionado invÃ¡lido.')
            return redirect('registrar_pedido')

        if estado not in estados_validos:
            messages.error(request, 'Estado de pedido invÃ¡lido.')
            return redirect('registrar_pedido')
        ...
        try:
            ...
            messages.error(request, 'Cantidad o precio invÃ¡lido.')
            return redirect('registrar_pedido')

        if cantidad_bidones < 1:
            messages.error(request, 'La cantidad mÃ­nima es 1 bidÃ³n.')
            return redirect('registrar_pedido')

        if cantidad_bidones > 100:
            messages.error(request, 'La cantidad mÃ¡xima permitida es 100 bidones.')
            return redirect('registrar_pedido')

        if precio_unitario < Decimal('1.00'):
            messages.error(request, 'El precio unitario mÃ­nimo permitido es S/ 1.00.')
            return redirect('registrar_pedido')

        if precio_unitario > Decimal('50.00'):
            messages.error(request, 'El precio unitario mÃ¡ximo permitido es S/ 50.00.')
            return redirect('registrar_pedido')
        ...
        if fecha_programada:
            try:
                ...
                messages.error(request, 'Fecha programada invÃ¡lida.')
                return redirect('registrar_pedido')
        ...
        if not cliente:
            messages.error(request, 'El cliente seleccionado no existe o estÃ¡ inactivo.')
            return redirect('registrar_pedido')
        ...
        if repartidor_id:
            if not repartidor_id.isdigit():
                messages.error(request, 'Repartidor seleccionado invÃ¡lido.')
                return redirect('registrar_pedido')
            ...
            if not repartidor:
                messages.error(request, 'El repartidor seleccionado no estÃ¡ activo.')
                return redirect('registrar_pedido')
...
def editar_pedido(request, pedido_id):
    ...
    if request.method == 'POST':
        ...
        if cantidad_bidones > 100:
            messages.error(
                request,
                'La cantidad mÃ¡xima permitida es 100 bidones.'
            )
            return redirect('editar_pedido', pedido_id=pedido.id)
        ...
        if precio_unitario < Decimal('1.00'):
            messages.error(
                request,
                'El precio unitario mÃ­nimo permitido es S/ 1.00.'
            )
            return redirect('editar_pedido', pedido_id=pedido.id)
        ...
        if precio_unitario > Decimal('50.00'):
            messages.error(
                request,
                'El precio unitario mÃ¡ximo permitido es S/ 50.00.'
            )
            return redirect('editar_pedido', pedido_id=pedido.id)
        ...
        if repartidor_id:
            ...
            if not repartidor:
                messages.error(
                    request,
                    'El repartidor seleccionado no estÃ¡ activo.'
                )
                return redirect('editar_pedido', pedido_id=pedido.id)
...
def lista_pedidos(request):
    ...
    if estado:
        ...
        if estado in estados_validos:
            ...
            messages.error(request, 'Filtro de estado invÃ¡lido.')
            estado = ''
    ...
    pedidos = pedidos.order_by(
        'prioridad_operativa',
        'fecha_programada_orden',
        'fecha_pedido_pendiente_orden',
        '-fecha_pedido_cerrado_orden'
    )
...
def nuevo_pedido_repartidor(request):
    ...
    if request.method == 'POST':
        ...
        if not cliente_id or not cliente_id.isdigit():
            messages.error(request, 'Debe seleccionar un cliente vÃ¡lido.')
        ...
        if entregar_ahora:
            messages.success(
                request,
                'Pedido rÃ¡pido registrado como entregado.'
            )
        else:
            messages.success(
                request,
                'Pedido rÃ¡pido registrado como pendiente.'
            )
...
def nuevo_cliente_repartidor(request):
    ...
    if request.method == 'POST':
        ...
        if len(limpiar_telefono(telefono)) < 6:
            messages.error(
                request,
                'El telÃ©fono debe tener al menos 6 dÃ­gitos.'
            )
            return redirect('nuevo_cliente_repartidor')
...
def cambiar_estado_pedido(request, pedido_id, nuevo_estado):
    ...
    if pedido.estado != Pedido.PENDIENTE:
        messages.error(request, 'Solo se pueden modificar pedidos pendientes desde esta acciÃ³n.')
        return redirect('lista_pedidos')
...
def revertir_entrega(request, pedido_id):
    ...
    messages.success(
        request,
        'Entrega revertida. El pedido volviÃ³ a estado PENDIENTE.'
    )
...
def reporte_mensual(request):
    ...
    if total_entregados == 0:
        ...
        lectura_mes = 'Hay mÃ¡s pedidos pendientes que entregados; conviene priorizar cierres.'
